...
public record DarkelpFeatureConfig(int spreadWidth, int spreadHeight, int maxHeight) implements FeatureConfig  {
    public static final Codec<DarkelpFeatureConfig> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(Codecs.POSITIVE_INT.fieldOf("spread_width").forGetter(DarkelpFeatureConfig::spreadWidth), Codecs.POSITIVE_INT.fieldOf("spread_height").forGetter(DarkelpFeatureConfig::spreadHeight), Codecs.POSITIVE_INT.fieldOf("max_height").forGetter(DarkelpFeatureConfig::maxHeight)).apply(instance, DarkelpFeatureConfig::new);
    });
